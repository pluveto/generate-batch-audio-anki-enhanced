...
def log(args):
    with open("audiodownloader.log", "a", encoding="utf-8") as f:
        f.write(str(args) + "\n")
...
def create_file_name(args_dict, relevant_keys, audio_ext):
    """
...
class AudioDownloader:
    ...
    def download_single(self, args_dict, duplicate_fld):
        ...
        """
        log("Downloading audio file for %s" % args_dict)
        # try each in order, if result is nothing or error occurs try next
        for audio_api_name, raw_url in self.audio_sources.items():
            try:
                ...
                output_file_name = create_file_name(args_dict, relevant_keys, audio_ext)
                ...
                ex_str = str(ex)
                log(f"Error downloading audio file for {args_dict}: {ex_str}")
                pass
