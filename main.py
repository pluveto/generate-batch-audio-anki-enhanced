...
class BulkGenerateDialog(QDialog):
    ...
    def saveConfig(self):
        ...
        config['ksel'] = self.filter_kana_cb.currentText()
    ...
    def loadConfig(self):
        ...
        if config['ksel'] and self.filter_kana_cb.findText(config['ksel']):
            self.filter_kana_cb.setCurrentText(config['ksel'])
        if config['dup_cb']:
    ...
    def _setupUi(self):
        ...
        self.filter_kana_cb = QComboBox()
        fields = self._getFields()
        self.filter_kana_cb.addItem("(None)")
        self.filter_kana_cb.addItems(fields)
        ...
        f_grid.addWidget(self.filter_kana_cb, 1, 2)
    ...
    def onGenerate(self):
        ...
        """
        filter_kana_fld = self.filter_kana_cb.currentText()
        ...
        mw = browser.mw
        nids = self.nids
        audio_field_name = self.fsel.currentText()
        ...
        op = QueryOp(
            parent=mw,
            op=lambda col: _processNotes(browser, nids, audio_field_name, sleep_time, filter_kana_fld, ad, duplicate_fld),
...
def _processNotes(browser, nids, audio_field_name, sleep_time, filter_kana_fld, ad, duplicate_fld):
    """
    ...
    # Step 1: Collect data for downloading
    notes_to_download = []
    for nid in nids:
        current_note = mw.col.getNote(nid)
        if audio_field_name in current_note:
            if current_note[audio_field_name].startswith('[sound:'):
                log(f"Note {nid} already has audio, skipping")
                continue

            note_dict = {}
            for key, value in current_note.items():
                if key == filter_kana_fld:
                    value = filter_kana(value)
                note_dict.update({key: value})

            notes_to_download.append((nid, note_dict))

    # Function to download audio
    def download_audio(nid, note_dict):
        if sleep_time > 0:
            time.sleep(sleep_time)

        audio_filename = ad.download_single(note_dict, duplicate_fld)
        if audio_filename:
            log(f"Downloaded audio {audio_filename} for note {nid}")
            return nid, audio_filename
        return nid, None

    # Step 2: Concurrently download audio files
    download_results = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=32) as executor:
        future_to_nid = {executor.submit(download_audio, nid, note_dict): nid for nid, note_dict in notes_to_download}
        for future in concurrent.futures.as_completed(future_to_nid):
            nid, audio_filename = future.result()
            if audio_filename:
                download_results[nid] = audio_filename

    # Step 3: Update notes with downloaded audio files
    for nid in nids:
        # Update progress
        ...
        if nid in download_results:
            current_note = mw.col.getNote(nid)
            current_note[audio_field_name] = '[sound:' + download_results[nid] + ']'
            current_note.flush()
            log(f"Updated note {nid} with audio {download_results[nid]}")
            cnt += 1
